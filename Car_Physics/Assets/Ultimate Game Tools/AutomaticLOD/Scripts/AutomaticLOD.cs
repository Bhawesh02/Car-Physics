...
public class AutomaticLOD : MonoBehaviour
{
  ...
  public int GetCurrentVertexCount(bool bRecurseIntoChildren, Camera currentCamera = null)
  {
    int LODGroupLevel = -1;
    
    if (m_LODGroup != null)
    {
      currentCamera = currentCamera ? currentCamera : Camera.current;
      ...
      LODGroupLevel = LODGroupExt.GetVisibleLOD(m_LODGroup, currentCamera);
    }

    int nVertexCount = 0;
    GetCurrentVertexCountRecursive(this, this.gameObject, ref nVertexCount, bRecurseIntoChildren, currentCamera, LODGroupLevel);
    return nVertexCount;
  }

  private static void GetCurrentVertexCountRecursive(AutomaticLOD root, GameObject gameObject, ref int nVertexCount, bool bRecurseIntoChildren, Camera camera, int LODGroupLevel)
  {
    AutomaticLOD automaticLOD = gameObject.GetComponent<AutomaticLOD>();
    ...
    if (automaticLOD != null)
    {
      if (IsRootOrBelongsToLODTree(automaticLOD, root))
      {
        if (LODGroupLevel != -1)
        {
          if (automaticLOD.m_listLODLevels != null && automaticLOD.m_listLODLevels[LODGroupLevel].m_mesh != null)
          {
            nVertexCount += automaticLOD.m_listLODLevels[LODGroupLevel].m_mesh.vertexCount;
          }
        }
        else
        {
    ...
    if (bRecurseIntoChildren)
    {
      for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
      {
        GetCurrentVertexCountRecursive(root, gameObject.transform.GetChild(nChild).gameObject, ref nVertexCount, bRecurseIntoChildren, camera, LODGroupLevel);
      }
    }
  }
