...
public class LODSampleLODScene : MonoBehaviour
{
	...
	void Start ()
  {
    ...
    m_objectMaterials = new Dictionary<Renderer, Material[]>();
    m_nMaxLODLevels = 0;
  ...
  void OnGUI()
  {
    ...
    foreach(AutomaticLOD automaticLOD in m_sceneLODObjects)
    {
      nLODVertexCount   += automaticLOD.GetCurrentVertexCount(true, m_selectedCamera.m_camera);
  ...
  private void AddMaterials(GameObject theGameObject, Dictionary<Renderer, Material[]> dicMaterials)
  {
    Renderer[] renderers = theGameObject.GetComponentsInChildren<Renderer>();

    foreach(Renderer renderer in renderers)
    {
      dicMaterials.Add(renderer, renderer.sharedMaterials);
    }
  }
  ...
  private void SetWireframe(bool bEnabled)
  {
    ...
    foreach (KeyValuePair<Renderer, Material[]> pair in m_objectMaterials)
    {
      if (bEnabled)
      {
  ...
  Dictionary<Renderer, Material[]>   m_objectMaterials;
