...
[FIX] Checked Unity 2019.1+ compatibility.

[FIX] Fixed issues where creating ImportedBones ropes would complain about not being
      able to re-parent nodes coming from a prefab instance (new Unity prefab workflow).
	  If nodes need to be re-parented then the prefab instance is unpacked.

V1.25 - 28/07/2017:

[FIX] Fixed a bug that threw exceptions when creating ImportedBones ropes.

[ADD] Added the RopeMaxTensionHandler component to handle the cases where the rope gets
      stretched beyond its length.
	  It allows to:
	    -Fix the haywire effect by stretching the bones linearly.
		-Make the rope end dynamic when the maximum tension has been reached. This
		 allows to create effects like unplugging a cord when pulling from a cable.
		-Have custom notifications for max tension start/continue/end.
