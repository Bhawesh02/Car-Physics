...
//          Debug.Log(nTriangleIndex + " " + nLinkVertexIndexStart + " " + nJumpVertices);
    ...
    bool BuildImportedBoneListTry(GameObject goRoot, string strPrefix, int nIndexFirst, int nIndexLast, int nDigitsFirst, int nDigitsLast, List<int> ListImportBonesStatic, List<int> ListImportBonesNoCollider, out List<RopeBone> outListImportedBones, ref string strErrorMessage)
    {
        ...
        foreach(RopeBone bone in outListImportedBones)
        {
            ...
            // Added since new Unity prefab workflow, because gameobjects can not be
            // re-parented if they are part of a prefab instance:

#if UNITY_EDITOR

            GameObject prefabRoot = UnityEditor.PrefabUtility.GetOutermostPrefabInstanceRoot(bone.goBone);

            if(prefabRoot != null)
            {
                UnityEditor.PrefabUtility.UnpackPrefabInstance(prefabRoot, UnityEditor.PrefabUnpackMode.Completely, UnityEditor.InteractionMode.AutomatedAction);
            }
