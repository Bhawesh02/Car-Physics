    ...
    public class MeshData
    {
        ...
        public class IncreasingSizeComparer : IComparer<MeshData>
        {
            ...
            public int Compare(MeshData a, MeshData b)
            {
                ...
                if(nSplitAxis == 0)
                {
                    float d = (v3MaxA.x - v3MinA.x) - (v3MaxB.x - v3MinB.x);
                    return d == 0.0f ? 0 : d < 0.0f ? -1 : 1;
                }
                else if(nSplitAxis == 1)
                {
                    float d = (v3MaxA.y - v3MinA.y) - (v3MaxB.y - v3MinB.y);
                    return d == 0.0f ? 0 : d < 0.0f ? -1 : 1;
                }
                else if(nSplitAxis == 2)
                {
                    float d = (v3MaxA.z - v3MinA.z) - (v3MaxB.z - v3MinB.z);
                    return d == 0.0f ? 0 : d < 0.0f ? -1 : 1;
                }

                float diff = Mathf.Max(v3MaxA.x - v3MinA.x, v3MaxA.y - v3MinA.y, v3MaxA.z - v3MinA.z) - Mathf.Max(v3MaxB.x - v3MinB.x, v3MaxB.y - v3MinB.y, v3MaxB.z - v3MinB.z);
